...
def test_visualize_from_database():
    ...
    if not interactions:
        pytest.skip(f"Database has no data for {protein}")

    # 3. Try to visualize (should create cache from database)
    print(f"\n3. Requesting visualization...")

    try:
        response = requests.get(f"{BASE_URL}/api/visualize/{protein}", timeout=5)
        ...
        if response.status_code == 200:
            ...
            pytest.fail("Visualization endpoint returned 404")

        else:
            print(f"   Status: {response.status_code}")
            print(f"   Response: {response.text[:200]}")
            pytest.fail(f"Visualization endpoint returned unexpected status {response.status_code}")

    except requests.exceptions.RequestException as exc:
        pytest.skip(f"Local Flask server unavailable: {exc}")
...
def test_full_instant_query_flow():
    ...
    try:
        ...
        if status != "complete":
            ...
            pytest.fail(f"Expected complete status, got {status!r}")

        # Step 2: Visualize
        print(f"\n2. Loading visualization...")
        viz_response = requests.get(f"{BASE_URL}/api/visualize/{protein}", timeout=5)
        ...
        if viz_response.status_code == 200:
            ...
            pytest.fail(f"Visualization failed with status {viz_response.status_code}")

    except requests.exceptions.RequestException as exc:
        pytest.skip(f"Local Flask server unavailable: {exc}")
    except Exception as e:
        pytest.fail(f"Instant query flow test failed: {e}")
