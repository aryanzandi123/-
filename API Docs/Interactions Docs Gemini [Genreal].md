...
When continuing a conversation using `previous_interaction_id`, the API remembers the conversation history. However, it treats the new request as a distinct configuration event. You must explicitly re-declare your configuration fields on subsequent turns. Here is how the API handles field inheritance when `previous_interaction_id` is provided:

| Field | Inherited? | What you need to do |
|---|---|---|
| Conversation history | ✅ Yes | You don't need to pass previous turns. Only pass the new input. |
| `model` / `agent` | ❌ No | **Required.** You must specify the model or agent in every request. If omitted, the request will fail. |
| `system_instruction` | ❌ No | You must re-send the system instructions on every turn if you want them to continue applying to the model's behavior. |
| `tools` | ❌ No | You must re-send the tools array on new user message turns if you want the model to retain its capabilities. ⚠️ **Important** : Don't send the tools array when your input is a `function_result`. Doing so will result in an API error. |
| `generation_config` | ❌ No | Parameters like temperature or max_output_tokens revert to defaults unless you explicitly re-send them in the new request. |
...
`content.delta` events on the stream. Since the `outputs` array in the final
`interaction.complete` event is empty, you must aggregate these deltas to reconstruct
the complete tool calls before executing them.
...
### Python
    ...
    for chunk in stream:
        if chunk.event_type == "content.delta":
            if chunk.delta.type == "function_call":
                call_id = chunk.delta.id
                if call_id not in function_call_deltas:
                    function_call_deltas[call_id] = {
                        "id": call_id,
                        "name": chunk.delta.name,
                        "arguments": ""
                    }
                if hasattr(chunk.delta, "arguments") and chunk.delta.arguments:
                    # Depending on SDK version, arguments could be dict or string
                    if isinstance(chunk.delta.arguments, str):
                        function_call_deltas[call_id]["arguments"] += chunk.delta.arguments
                    else:
                        function_call_deltas[call_id]["arguments"] = chunk.delta.arguments
            ...
            # Reconstruct outputs array from aggregated map
            if not outputs and function_call_deltas:
                for call_id, d in function_call_deltas.items():
                    args = d["arguments"]
...
### JavaScript
    ...
    const functionCallDeltas = new Map();
    let outputs = [];
    ...
    for await (const chunk of stream) {
        if (chunk.event_type === 'content.delta') {
            if (chunk.delta.type === 'function_call') {
                ...
                if (!functionCallDeltas.has(id)) {
                    functionCallDeltas.set(id, {
                        id: id,
                        name: chunk.delta.name,
                        arguments: ''
                    });
                ...
                if (chunk.delta.arguments) {
                    functionCallDeltas.get(id).arguments += chunk.delta.arguments;
                }
            } else if (chunk.delta.type === 'text' && 'text' in chunk.delta) {
                process.stdout.write(chunk.delta.text);
            }
        } else if (chunk.event_type === 'interaction.complete') {
            // Reconstruct outputs array from aggregated map
            if (outputs.length === 0 && functionCallDeltas.size > 0) {
                outputs = Array.from(functionCallDeltas.values()).map(d => ({
                    type: 'function_call',
                    id: d.id,
                    name: d.name,
                    arguments: d.arguments ? JSON.parse(d.arguments) : {}
                }));
            }
            console.log('\nFinal tool calls ready to execute:');
...
| Gemini 3 Pro Preview | Model | `gemini-3.1-pro-preview` |
