...
PRUNE_MODEL = os.getenv("PRUNE_MODEL", "gemini-3-flash-preview")
...
def _call_gemini_json(prompt: str, max_retries: int = 3) -> dict:
    """
    Call Gemini 3 Pro without Google Search (fast), parse strict JSON.

    H5: api_key parameter removed. The pipeline runs on Vertex AI with
    Application Default Credentials; ``get_client()`` validates project
    + location from environment via ``validate_vertex_config()``.
    """
    client = get_client()
    ...
    for attempt in range(1, max_retries+1):
        try:
            ...
            output_text = extract_text_from_generate_response(resp)
            if output_text:
                return extract_json_from_llm_response(output_text)
...
def llm_select_keep_list(
    ...
    H5: api_key parameter removed. Vertex AI auth is gated by
    ``validate_vertex_config()`` inside ``get_client``; if the env vars
    are missing the LLM call raises and we fall through to the
    deterministic fallback selector.
    """
    if use_llm:
        try:
            prompt = build_pruning_prompt(parent, protein, main, candidates, hard_max_keep, current_nodes)
            # validate_vertex_config raises with a clear message when
            # GOOGLE_CLOUD_PROJECT / GOOGLE_CLOUD_LOCATION are missing.
            from utils.gemini_runtime import validate_vertex_config
