...
try:
    ...
    print("[WARN] Could not import aggregate_function_arrows from utils.aggregation - some fixes disabled")
...
def _reaggregate_interactor(interactor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Fallback: lightweight aggregation (matches utils/aggregation.py logic)
    functions = interactor.get('functions', []) or []
    arrows_by_dir = { 'main_to_primary': set(), 'primary_to_main': set() }
    dir_counts = { 'main_to_primary': 0, 'primary_to_main': 0 }
    func_names_by_dir = { 'main_to_primary': set(), 'primary_to_main': set() }

    for fn in functions:
        if not isinstance(fn, dict):
            continue
        eff = (fn.get('interaction_effect') or fn.get('arrow') or 'binds')
        d = (fn.get('interaction_direction') or fn.get('direction') or '')
        ...
        d = str(d).lower()
        if 'bid' in d:
            # Normalize invalid direction to both canonical directions
            dir_counts['main_to_primary'] += 1
            dir_counts['primary_to_main'] += 1
            arrows_by_dir['main_to_primary'].add(eff)
            arrows_by_dir['primary_to_main'].add(eff)
            func_names_by_dir['main_to_primary'].add(func_name)
            func_names_by_dir['primary_to_main'].add(func_name)
            continue
        elif 'primary_to_main' in d or d == 'p2m' or d == 'b_to_a':
            d = 'primary_to_main'
        elif 'main_to_primary' in d or d == 'm2p' or d == 'a_to_b':
            d = 'main_to_primary'
        elif d:
            d = 'main_to_primary'
        else:
            d = 'main_to_primary'  # Only default here as last resort for fallback

        dir_counts[d] = dir_counts.get(d, 0) + 1
        arrows_by_dir[d].add(eff)
        func_names_by_dir[d].add(func_name)
    ...
    # Direction - pick dominant when both directions present
    m2p = dir_counts.get('main_to_primary', 0)
    p2m = dir_counts.get('primary_to_main', 0)
    m2p_unique = func_names_by_dir['main_to_primary'] - func_names_by_dir['primary_to_main']
    p2m_unique = func_names_by_dir['primary_to_main'] - func_names_by_dir['main_to_primary']

    if bool(m2p_unique) and bool(p2m_unique) and m2p > 0 and p2m > 0:
        # Functions in both directions — pick dominant. DB CHECK blocks "bidirectional".
        direction = 'primary_to_main' if p2m >= m2p else 'main_to_primary'
    elif p2m > m2p:
        direction = 'primary_to_main'
    elif m2p > p2m:
        direction = 'main_to_primary'
    elif p2m == m2p and p2m > 0:
        direction = 'primary_to_main'  # Tie → conservative (assume interactor acts on query)
    else:
        direction = 'main_to_primary'
...
def validate_schema_consistency(
    ...
    - Incorrect direction classifications
    ...
    def _dedupe_functions(funcs: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], int]:
        ...
        for fn in funcs or []:
            ...
            # Normalize function-level direction if present
            if fn.get("direction"):
                fn["direction"] = _normalize_fn_direction(fn.get("direction"))
            # Mirror between interaction_direction and direction when one is missing
            # but do NOT default to main_to_primary when BOTH are missing
            # (let the arrow determination step fill it in)
            if not fn.get("interaction_direction") and fn.get("direction"):
                fn["interaction_direction"] = fn["direction"]
            elif fn.get("interaction_direction") and not fn.get("direction"):
                fn["direction"] = fn["interaction_direction"]
            # If BOTH are missing, leave them empty - don't assume main_to_primary
    ...
    for interactor in interactors:
        ...
        # Canonicalize functions (no dedup — that's the LLM dedup stage's
        # job downstream). Keeping the dedup here was double-counting:
        # this stage uses (name, arrow, direction, cellular_process) as
        # key while the LLM stage compares mechanism overlap. The two
        # disagreed: the schema gate would silently drop functions the
        # LLM stage would have merged with evidence preservation, OR
        # keep functions the LLM stage rightly removed. Single source of
        # truth = LLM dedup.
        if functions:
        ...
        if fix_arrows and functions and not interactor.get('_arrow_validated'):
            ...
            if missing_arrow_funcs:
                ...
                # Use interactor-level direction as default, not always main_to_primary
                interactor_direction = interactor.get('direction', 'main_to_primary')
                for func in missing_arrow_funcs:
                    if not func.get('arrow'):
                        func['arrow'] = 'regulates'
                    if not func.get('direction'):
                        func['direction'] = interactor_direction

                issues_fixed += 1
                if verbose:
                    print(f"    [FIX] Defaulted missing arrows to 'regulates' (neutral)")
        ...
        if fix_chains and interaction_type == 'indirect':
            ...
            # CASE 3: Only mediator_chain is missing
            elif has_upstream and not has_chain:
                issues_found += 1
                if verbose:
                    print(f"  [ISSUE] {primary}: Indirect interactor missing mediator_chain")

                # DO NOT auto-fill mediator_chain = [upstream]. That fill-in
                # forces every incomplete chain to length 3 (query → upstream
                # → primary) regardless of real biology. Leaving the chain
                # empty lets downstream code surface the incomplete-data
                # state instead of fabricating a plausible-looking lie.
                if verbose:
                    upstream = interactor.get('upstream_interactor')
                    print(
                        f"    [SKIP] NOT inferring mediator_chain from "
                        f"upstream={upstream!r} — refusing length-3 fill-in."
                    )
...
def finalize_interaction_metadata(
    ...
    for interactor in interactors:
        ...
        if add_arrow_notation:
            ...
            # For direction-split interactors, add a tag to help frontend differentiate
            if interactor.get('_direction_split'):
                split_dir = interactor.get('_split_direction', direction)
                if split_dir == 'primary_to_main':
                    interactor['_split_label'] = 'upstream'
                else:
                    interactor['_split_label'] = 'downstream'
...
def validate_interactor_functions(
    ...
    for i, func in enumerate(functions):
        ...
        # Validate direction — inherit from interactor-level if invalid
        direction = func.get('direction', '')
        if direction not in valid_directions:
            interactor_direction = interactor.get('direction', 'main_to_primary')
            fallback = interactor_direction if interactor_direction in valid_directions else 'main_to_primary'
            if verbose:
                print(f"  [WARN] {primary} function {i}: Invalid direction '{direction}' - defaulting to '{fallback}'")
            func['direction'] = fallback
...
def print_validation_report(json_data: Dict[str, Any]) -> None:
    ...
    other = sum(1 for i in interactors if i.get('direction') not in ('main_to_primary', 'primary_to_main'))
    ...
    print(f"  other/unknown: {other} ({100*other/len(interactors):.1f}%)")
