...
Runs runner.py → evidence_validator.py → visualizer.py
...
def main():
    parser = argparse.ArgumentParser(
        description="Complete pipeline: runner.py → evidence_validator.py → visualizer"
    )
    parser.add_argument(
        "protein",
    ...
    parser.add_argument(
        "--skip-runner",
        action="store_true",
        help="Skip runner.py and use existing JSON"
    )
    parser.add_argument(
        "--skip-validator",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Core request mode forwarded to runner.py."
    )
    parser.add_argument(
        "--batch-poll-seconds",
        type=int,
        help="Batch polling interval forwarded to runner.py."
    )
    parser.add_argument(
        "--batch-max-wait-seconds",
        type=int,
        help="Batch max wait forwarded to runner.py."
    )
    ...
    if not args.skip_runner:
        ...
        if args.request_mode:
            cmd += ["--request-mode", args.request_mode]
        if args.batch_poll_seconds is not None:
            cmd += ["--batch-poll-seconds", str(args.batch_poll_seconds)]
        if args.batch_max_wait_seconds is not None:
            cmd += ["--batch-max-wait-seconds", str(args.batch_max_wait_seconds)]

        step_times['runner'] = run_command(cmd, "1. Running pipeline (runner.py)")
    else:
        if not runner_output.exists():
            print(f"❌ ERROR: {runner_output} not found and --skip-runner was specified")
            sys.exit(1)
        print(f"⏭️  Skipping runner.py, using existing: {runner_output}")
    ...
    # Step 3: Copy final JSON to [PROTEIN].json
    print(f"\n{'='*80}")
    ...
    # Step 4: Citation finding is now handled by PostProcessor stage
    # (utils/citation_finder.py via runner.py). Legacy CLI path skipped.
    print(f"\n{'='*80}")
    print(f"STEP: Skipping legacy PMID refresh (handled by PostProcessor citation finder)")
    print(f"{'='*80}")
    step_times['pmid_refresh'] = 0.0

    data = load_json(final_json)

    # Step 5: Run visualizer.py
    if not args.skip_viz:
        ...
        step_times['visualizer'] = run_command(cmd, "4. Generating visualization (visualizer.py)")
    else:
    ...
    if 'runner' in step_times:
        print(f"    1. Runner (pipeline):          {step_times['runner']:.1f}s ({step_times['runner']/60:.1f} min)")
    ...
    if 'pmid_refresh' in step_times:
        print(f"    3. PubMed refresh:             {step_times['pmid_refresh']:.1f}s ({step_times['pmid_refresh']/60:.1f} min)")
    if 'visualizer' in step_times:
        print(f"    4. Visualizer:                 {step_times['visualizer']:.1f}s ({step_times['visualizer']/60:.1f} min)")
